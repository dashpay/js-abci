...
class Connection extends EventEmitter {
  ...
  async write(response) {
    ...
    const responseLengthBuffer = Buffer.from(
      ...
      varint.encode(responseBuffer.length),
    );
  ...
  async readNextRequest() {
    ...
    try {
      // eslint-disable-next-line no-bitwise
      requestLength = varint.decode(this.readerBuffer.slice(0, 8));
    } catch (e) {
    ...
    try {
      ...
      /*
      * Since the implementation of protobuf in TenderDash omits default values,
      *  and the implementation in protobufjs does not substitute missing values,
      * we use this construction to restore the default values
      * */
      request = Request.fromObject(Request.toObject(request, { defaults: true }));
    } catch (e) {
